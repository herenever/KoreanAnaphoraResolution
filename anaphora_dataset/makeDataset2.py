            ...
            for ZA in ZAs:
                ...
                if antecedent_sentence_id == '-1':
                    if n_cnt <=3000:
                        ...
                        window_size_sentence = sentences[cnt-1]['form']
                        length = len(predicate_sentence+window_size_sentence) + 1
                        begin = 0
                        end = 0
                        antecedent_word ="불필요"
                        n_cnt+=1
                    ...
                    if sentences[cnt-1]['id'] != antecedent_sentence_id:
                        # re_cnt = 0
                ...
                if predicate_sentence !="" and window_size_sentence != "" and length <= 500:
                    all_cnt +=1
                    with open('./anaphora_dataset/new_dataset3.csv','a') as cf:
                        wr = csv.writer(cf)
...
# f = open('./anaphora_dataset/new_dataset3.csv','w',newline='')
# wr = csv.writer(f)
...
data = pd.read_csv('./anaphora_dataset/statement.csv',encoding='UTF8')
...
train.to_csv('./anaphora_dataset/state_train.csv',index=False)
validation.to_csv('./anaphora_dataset/state_validation.csv',index=False)
